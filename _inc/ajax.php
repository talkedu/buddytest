...
function cb_the_status( $args = false ) {
	/***
	...
	if ( 'new' == $_GET['status'] && is_user_logged_in() ) {
		locate_template( array( 'members/single/status/post-form.php' ), true );
	} else {
		echo cb_get_the_status( $args );
	}
}
	function cb_get_the_status( $args = false ) {
		global $bp;
		...
		$status =bp_get_activity_latest_update( bp_displayed_user_id() );//apply_filters( 'bp_status_from_usermeta', get_usermeta( $user_id, 'bp_status' ) );
	
		if ( empty($status) )
			return cb_get_update_status_button( 'text=' . $new_button_text );

		//$time_since = sprintf( __( '%s ago', 'buddypress' ), bp_core_time_since( $status['recorded_time'] ) );
		$content = apply_filters( 'the_status_content', $status );
		...
		$content .= ' ' . cb_get_clear_status_button( 'text=' . $clear_button_text );

		return apply_filters( 'bp_get_the_status', $content );
	}

function cb_update_status_button( $args = false ) {
	echo cb_get_update_status_button( $args );
}
	function cb_get_update_status_button( $args = false ) {
		global $bp;
...
function cb_clear_status_button( $args = false ) {
	echo cb_get_clear_status_button( $args );
}
	function cb_get_clear_status_button( $args = false ) {
		global $bp;
...
function cb_status_clear_status( $user_id ){
//remove the last entry from the usermet
delete_usermeta($user_id,"bp_latest_update");
echo cb_get_the_status();
}
function cb_the_status_css_class() {
	echo cb_get_the_status_css_class();
}
	function cb_get_the_status_css_class() {
...
function bp_dtheme_ajax_new_status() {
	...
	if ( !check_admin_referer( 'post_update' ) )
		return false;
		
	if ( cb_status_add_status( $bp->loggedin_user->id, $_POST['status-update-input'] ) )
		echo cb_get_the_status();
	else
		echo "-1";
}
...
function bp_dtheme_ajax_clear_status( $new_text = false ) {
	...
	cb_status_clear_status( $bp->loggedin_user->id );
...
function cb_status_add_status($user_id,$content )
{
bp_activity_post_update( array('user_id'=>$user_id, 'content' => $content ) );
return true;
}
?>